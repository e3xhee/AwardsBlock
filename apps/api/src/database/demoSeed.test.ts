...
type AwardBlockListResponse = {
  awardBlocks: Array<{
    ...
    award: { id: string; title: string; status: string };
...
async function withSeededApi(run: (baseUrl: string, database: DatabaseSync) => Promise<void>) {
...
test("demo seed exposes a complete award and claim journey", async () => {
  await withSeededApi(async (baseUrl, database) => {
    ...
    const awardBlock = list.awardBlocks.find((candidate) => candidate.id === "award-1");

    assert.ok(awardBlock);
    assert.equal(awardBlock.event.name, "Seoul Demo Day");
    assert.equal(awardBlock.project.name, "ProofBoard");
    assert.equal(awardBlock.award.status, "Claiming");
    assert.deepEqual(awardBlock.claimStats, { recipientCount: 2, claimedCount: 1 });
    ...
    assert.equal(detail.awardBlock.members.find((member) => member.id === "member-1")?.inviteStatus, "Claimed");
    assert.equal(detail.awardBlock.members.find((member) => member.id === "member-2")?.inviteStatus, "Invited");
    assert.ok(detail.awardBlock.transactions.some((transaction) => transaction.transactionType === "AwardClaimed"));

    const inviteResponse = await fetch(`${baseUrl}/claim-invites/demo-claim-token`);
    ...
    assert.equal(profile.profile.awards[0]?.member.claimTxHash, "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
  });
});
